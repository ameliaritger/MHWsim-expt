...
#Load data
```{r}
temp_raw <-  read_csv(here("data", "mohawk_mooring_mko_20230608.csv"), col_names = TRUE) %>%
...
mhw_seas <- mhw$climatology %>%
  slice(mhw_start:mhw_end) %>%
  mutate(month=as.factor(month(t))) %>%
  filter(t>=ymd("2015-08-15") & t<ymd("2016-03-01")) %>%
  select(t, seas)
```
...
joined_df <- left_join(mhw_profile, mhw_seas, by = "t") %>%
  ...
  mutate(mean_temp = mean(temp),
         temp_diff=mean_temp-seas,
         temp_diff_thresh=mean_temp-seas_thresh,
         temp_diff_extreme=mean_temp-seas_extreme,
         adjusted_temp=temp-temp_diff,
         adjusted_thresh=temp-temp_diff_thresh,
         adjusted_extreme=temp-temp_diff_extreme) %>%
  ungroup()

#Plot it all, compare transformed "Blob" data with real Blob data
ggplot(joined_df, aes(x = date_time)) +
  geom_line(aes(y = temp, colour="temp"), size = 0.8) +
  ...
  geom_line(data = joined_df[!is.na(joined_df$seas), ],aes(y = seas, colour="seas"),size = 0.8) +
  scale_colour_manual(name = "Line Colour",
                      values = c("temp" =  "orange",
                                 "adjusted_thresh" = "red", 
                                 "adjusted_extreme" =  "darkred",
                                 "adjusted_temp" =  "purple",
                                 "seas" = "dodgerblue4"),
                      labels= c("Transformed extreme", "Transformed climatology", "Transformed Blob", "Climatology","The Blob")) +
  scale_x_datetime(date_labels = "%b %d",
                   breaks = scales::date_breaks("1 week"),
                   expand = c(0, 0),
                   limits = c(min(joined_df$date_time),max = max(joined_df$date_time))) +
  scale_y_continuous(limits = c(13, 27), breaks = seq(13, 27, by = 2)) +
  guides(colour = guide_legend(override.aes = list(fill = NA))) +
...
#ggsave(here("figures", "MHWprofile_all2.png"), width = 20, height = 7)

mhw_profile_final <- joined_df %>%
  select(date_time, chill=adjusted_temp, blob=adjusted_thresh, extreme=adjusted_extreme)

write_csv(mhw_profile, here("data", "mhw_profile_final.csv"))
```
