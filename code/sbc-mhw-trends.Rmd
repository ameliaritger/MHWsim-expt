...
#Load data
```{r}
sterns <-  read_csv(here("data", "sternswharf_mooring_sbh_20230608.csv"), col_names = TRUE) %>%
...
```{r}
event_list <- list(arq_event, carp_event, mko_event, nap_event)
...
mhw_profile <- nap_clean %>% #or use "all_data"
  filter(date>=ymd("2015-08-01") & date<ymd("2016-04-01")) %>%
  mutate(t = date,
         time=format(times(decimal_time))) %>%
  unite(date_time, c(t, time), sep = " ", remove = TRUE) %>% # combine the date and time columns
  mutate(date_time=ymd_hms(date_time)) %>%
...
ggplot(mhw_profile, aes(x=date_time)) +
  geom_point(aes(y=temp), color="orange")
  #geom_point(aes(y=chill), color="blue")

#get the seasonal climatology data
...
#ggsave(here("figures", "joineddf.png"), width = 20, height = 7)
