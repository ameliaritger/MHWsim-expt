...
# Are you Sam? Make this TRUE if this is you.
sam <- TRUE

if (sam) {
  knitr::opts_chunk$set(root.dir = '~/Documents/GitHub/MHWsim-expt/')
} else {knitr::opts_chunk$set(echo = TRUE)
}
...
**"n" = n_rel AKA (n_true)/first(n_true), since the initial number of polyps differed for every replicate**
**"behavior" = percent_fully_open, since that's what I'm using for other analysis**
**"size" = "avg_size" (in mm, not g) since that is the direct measurement rather than the converted measurement (g is a proxy but mm is the most accurate)**

```{r, eval=FALSE}

# First pass non time-series, Bayesian model
sem1_rand <- brm(
  bf(scale(avg_size) ~ scale(n_rel) + scale(avg_temp) + scale(percent_fully_open)) +
    bf(scale(n_rel) ~ scale(avg_temp) + (1|genet)) +
    bf(scale(percent_fully_open) ~ scale(avg_temp)) +
...
sem1_rand_temporal <- brm(
  bf(scale(avg_size) ~ scale(n_rel) + scale(avg_temp) + scale(percent_fully_open) +
       scale(avg_temp) : scale(n_rel) +
       gp(date_num, c = 1)) + 
  bf(scale(n_rel) ~ scale(avg_temp) + 
       (1 | tank/genet) + 
       gp(date_num, c = 1)) +
  bf(scale(percent_fully_open) ~ scale(avg_temp) + 
       gp(date_num, c = 1)) +
