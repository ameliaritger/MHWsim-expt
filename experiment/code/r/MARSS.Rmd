...
all_hypotheses$H2$Z[1:25, 1] <- 1  # Treatment 1
all_hypotheses$H2$Z[26:50, 2] <- 1  # Treatment 2
all_hypotheses$H2$Z[51:75, 3] <- 1  # Treatment 3
colnames(all_hypotheses$H2$Z) <- c("treatment 1", "treatment 2", "treatment 3")
all_hypotheses$H2$C[1, 1:5] <- "treatment1"
all_hypotheses$H2$C[2, 6:10] <- "treatment2"
all_hypotheses$H2$C[3, 11:15] <- "treatment3"

# H3: All genets are different -> Z (75x5), C (5x15)
fill_staggered_ones <- function(Z) {
  for (col in 1:ncol(Z)) {
...
all_hypotheses$H3$Z <- fill_staggered_ones(all_hypotheses$H3$Z)
for (i in 1:nrow(all_hypotheses$H3$C)) {
  all_hypotheses$H3$C[i, ] <- rep(letters[i], ncol(all_hypotheses$H3$C))
}

# H4: MHW treatments are different from control -> Z (75x2), C (2x15)
all_hypotheses$H4$Z[1:25, 1] <- 1  # Treatment 1
all_hypotheses$H4$Z[26:75, 2] <- 1  # Treatment 2
colnames(all_hypotheses$H4$Z) <- c("Ambient", "MHW")
all_hypotheses$H4$C[1, 1:5] <- "ambient"
all_hypotheses$H4$C[2, 6:15] <- "mhw"
...
for (hypothesis_number in names(all_hypotheses)) {
  # Extract the current Z and C combination
  Z <- all_hypotheses[[hypothesis_number]]$Z
  C <- all_hypotheses[[hypothesis_number]]$C
