...
temp_sorted <- temp_clean %>%
  arrange(timestamp) %>%
  mutate(week = ceiling_date(timestamp, unit = "week"))
         

%>%  # Extract the week starting date
  group_by(tank, week) %>%
  summarize(avg_temperature = mean(temperature, na.rm = TRUE))
  
plot_data <- temp_sorted %>%
  group_by(week) %>%
  summarize(
    start_date = floor_date(min(timestamp), unit = "day"),  # Set start_date to the start of the day
    end_date = ceiling_date(max(timestamp), unit = "day")- seconds(1),
    #start_date = min(timestamp) + days(1),
    #end_date = max(timestamp) + days(1),
    .groups = 'drop'
  ) %>%
  mutate(start_date = as_datetime(start_date))

ggplot(plot_data, aes(x = start_date, xend = end_date, y = week, yend = week)) +
  geom_segment(size = 1.5, color = "blue") +
  labs(
    title = "Date Ranges Covered by Each Week",
    x = "Date",
    y = "Week Start Date"
  ) +
  theme_minimal() +
  scale_x_datetime(date_labels = "%b %d, %Y", date_breaks = "4 days") +
  scale_y_datetime(date_labels = "%Y-%m-%d", date_breaks = "1 week") +
