...
ggplot(mko_mhw, aes(x = t, ymax = pmax(thresh, temp), ymin = thresh)) +
  ...
  annotate("rect", xmin=ymd("2018-07-01"), xmax=ymd("2018-07-31"), ymin=-Inf, ymax=Inf, alpha=0.2,fill="#999999") +
  #geom_text(size = 12, colour = "black", aes(x = as.Date("2014-02-01"), y = 24, label = "MKO")) +
  #geom_line(aes(y = seas, colour = "seas"), size = 0.8, linetype="dashed") +
  scale_colour_manual(name = "Line Colour",
  ...
  scale_y_continuous(limits = c(10, 21), breaks = seq(10, 21, by = 2)) +
  guides(colour = guide_legend(override.aes = list(fill = NA))) +
  labs(y = expression(paste("Temperature (", degree, "C)")),
       x = "Date") +
  theme_bw() +
...
#m <- 
ggplot(mko_event, aes(x = date_time, y=temp_adcp, color=threshCriterion, group=factor(c(0, cumsum(abs(head(threshCriterion, -1) != tail(threshCriterion, -1))))))) +
  geom_line(alpha=0.9, size=0.7) +
  scale_x_datetime(date_labels = "%b %d",
                   breaks = scales::date_breaks("1 week"),
                   expand=c(0,0),
                   limits = c(min(mko_daily$date_time),
                              max(mko_daily$date_time))) +
...
ggsave(here("experiment", "figures", "mhw_month.png"), width=8, height=6)
```
