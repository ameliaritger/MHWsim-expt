...
ggplot(mko_mhw, aes(x = t, ymax = pmax(thresh, temp), ymin = thresh)) +
  ...
  geom_line(aes(y = seas, colour = "seas"), size = 1.5) +
  geom_ribbon(fill = "#D55E00") +
  geom_line(aes(y = thresh, colour = "thresh"), size = 1.5) +
  geom_line(aes(y = temp, colour = "temp"), alpha=0.5, size=1.5) +
  #geom_text(size = 12, colour = "black", aes(x = as.Date("2014-02-01"), y = 24, label = "MKO")) +
  #geom_line(aes(y = seas, colour = "seas"), size = 0.8, linetype="dashed") +
  scale_colour_manual(name = "Line Colour",
  ...
  theme(axis.text.x=element_text(angle=45, vjust = 1.12, hjust=1.1, size=18),
...
#ggsave(here("experiment", "figures", "dissertation", "mhw_year.png"), width=10, height=5)
...
ggplot(adcp_event, aes(x = date_time, y=temp_adcp, color=threshCriterion, group=factor(c(0, cumsum(abs(head(threshCriterion, -1) != tail(threshCriterion, -1))))))) +
  geom_line(aes(linetype=threshCriterion), alpha=0.9, size=1.5) +
  ...
  scale_linetype_manual(values = c("solid", "longdash"),
                        guide = "none") +
  scale_color_manual(values = c("#000000","#086788"),
                      labels= c("No", "Yes")) +
  labs(y = expression(paste("Temperature (", degree, "C)")),
       x = "",
...
ggsave(here("experiment", "figures", "dissertation", "mhwReal.png"), width=8, height=5) #or width = 8
