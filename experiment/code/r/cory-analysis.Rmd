...
For corAR1 models, might need to change to form = ~ date_num|genet_tank because observations aren't equally spaced across time.
...
ggsave(here("experiment", "figures", "dissertation", "behavior2.png"), height=6, width=12)
```
...
overdispersion_by_group <- all_count %>%
  group_by(genet_tank) %>%
  summarise(mean_n_true = mean(n_true, na.rm = TRUE),
...
# TRY THIS with AR(1) structure, like with growth (below)!

## Ah shit, we need to normalize the number of ramets because different starting numbers
```{r}
all_count <- all_count %>%
  group_by(genet) %>%
...
hist(log(all_count$n_rel))
```

```{r}
options <- c("treatment", "genet", "treatment+genet", "treatment*genet", "treatment+day_numeric", "genet+day_numeric", "treatment*day_numeric", "genet*day_numeric", "treatment+genet+day_numeric", "treatment*genet*day_numeric", "treatment*genet+day_numeric", "genet+treatment*day_numeric", "treatment+genet*day_numeric")

# Initialize an empty data frame to store the results
bic_table3 <- data.frame(model = character(), BIC = numeric(), AIC = numeric(), stringsAsFactors = FALSE)

for (option in options) {
    formula <- as.formula(paste("log(n_rel) ~", option))

    # Fit the model with the current option and AR(1) correlation
    model <- tryCatch({
        lme(formula,
            random = ~1 | genet_tank,
            data = all_count,
    ...
    # If the model successfully fitted, extract BIC, AIC, and df
    if (!is.null(model)) {
        bic_value <- BIC(model)
        aic_value <- AIC(model)
        
        # Add the model name, BIC, AIC, and df to the table
        bic_table3 <- rbind(bic_table3, data.frame(model = option, BIC = bic_value, AIC = aic_value))
    } else {
        # If there's an error, store NA for BIC, AIC, and df
        bic_table3 <- rbind(bic_table3, data.frame(model = option, BIC = NA, AIC = NA))
    }
}
```

## Post hoc test on "best" model
```{r}
model_repro1 <- lme(n_rel_log ~  treatment + day_numeric , random = ~1 | genet_tank, data = all_count, correlation = corAR1(form = ~ 1 | genet_tank))

#model_repro2 <- lme(n_rel_log ~  genet + day_numeric , random = ~1 | genet_tank, data = all_count, correlation = corAR1(form = ~ 1 | genet_tank))

#model_repro3 <- lme(n_rel_log ~  treatment + genet + day_numeric , random = ~1 | genet_tank, data = all_count, correlation = corAR1(form = ~ 1 | genet_tank))

#anova(model_repro1, model_repro3)
summary(model_repro)

#emm_trends <- emtrends(model_repro, specs = "genet", var = "day_numeric")
#emm_trends
emm_genet <- emmeans(model_repro, specs = "treatment")
emm_genet
pairs(emm_genet, adjust = "tukey") # Tukey is a common method for pairwise comparisons

model_repro2 <- lme(n_rel_log ~  genet * day_numeric , random = ~1 | genet_tank, data = all_count, correlation = corAR1(form = ~ 1 | genet_tank))

summary(model_repro2)
emm_trends_repro <- emtrends(model_repro2, specs = "genet", var = "day_numeric")
emm_trends_repro
pairs(emm_trends_repro, adjust = "tukey")
```


## Plot it
```{r}
preds <- as.data.frame(ggpredict(model_repro, terms = c("day_numeric", "treatment")))

preds <- preds %>%
  mutate(date = as.Date("2023-10-06") + x) %>%
  rename("treatment"="group")

ggplot() +
  #geom_ribbon(data=preds, aes(x = date, ymin = exp(conf.low), ymax = exp(conf.high), fill=treatment), alpha = 0.4) +
  geom_point(data=all_count, aes(x=date, y=n_rel, color=genet), size=2, alpha=0.6) +
  #geom_line(data=preds, aes(x = date, y = exp(predicted), color=treatment), size = 1.2) +
  geom_smooth(data=all_count, aes(x=date, y=n_rel, color=genet), method = "lm", linetype="dashed", se = FALSE, size = 2) +
  facet_wrap(~treatment,
             labeller =  labeller(treatment = c("cold" = "Ambient", "severe" = "Severe MHW", "extreme" = "Extreme MHW"))) +
  scale_x_date(date_labels = "%b %d",
               breaks = scales::date_breaks("2 weeks"),
               limits = c(min(all_count$date), max = max(all_count$date))) +
  scale_fill_manual(values = c("A" = "#9370DB", "B" = "#C21B78", "C" = "#FF9933", "D" ="#FF3333", "E" = "#662B45", "cold" = "#0072B2", "severe" = "#E69F00", "extreme" = "#D55E00"),
                    breaks = c("cold", "severe", "extreme"),
                    labels = c("cold" = "Ambient", "severe" = "Severe", "extreme" = "Extreme")) +
  scale_color_manual(values = c("A" = "#9370DB", "B" = "#C21B78", "C" = "#FF9933", "D" ="#FF3333", "E" = "#662B45", "cold" = "#0072B2", "severe" = "#E69F00", "extreme" = "#D55E00"),
                     breaks = c("A", "B", "C", "D", "E")) +
  labs(x = "",
       y = "Number of ramets (Standardized)",
       color = "Genet",
       fill = "Treatment") +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, vjust = 1, hjust=1, size=22),
...
ggsave(here("experiment", "figures", "dissertation", "standardized_polyps2.png"), height=8, width=12)
```
...
```{r}
options <- c("treatment", "genet", "treatment+genet", "treatment*genet", "treatment+day_numeric", "genet+day_numeric", "treatment+genet+day_numeric", "treatment*genet*day_numeric", "treatment*genet+day_numeric")
p_values <- 0:3  # Values for p
q_values <- 0:2  # Values for q
  ...
  for (p in p_values) {
    for (q in q_values) {
      ...
      if (!is.null(model)) {
        bic_value <- BIC(model)
        aic_value <- AIC(model)
        #df_value <- model$df # Extract degrees of freedom

        # Add the model name, p, q, BIC, AIC, and df to the table
        bic_table <- rbind(bic_table, data.frame(model = option, p = p, q = q, BIC = bic_value, AIC = aic_value))
      } else {
        # If there's an error, store NA for BIC, AIC, and df
        bic_table <- rbind(bic_table, data.frame(model = option, p = p, q = q, BIC = NA, AIC = NA))
...
# For loop through all fixed effect options
```{r}
options <- c("treatment", "genet", "treatment+genet", "treatment*genet", "treatment+day_numeric", "genet+day_numeric", "treatment*day_numeric", "genet*day_numeric", "treatment+genet+day_numeric", "treatment*genet*day_numeric", "treatment*genet+day_numeric", "genet+treatment*day_numeric", "treatment+genet*day_numeric")

# Initialize an empty data frame to store the results
bic_table2 <- data.frame(model = character(), BIC = numeric(), AIC = numeric(), stringsAsFactors = FALSE)
...
for (option in options) {
    ...
    if (!is.null(model)) {
        ...
        # Add the model name, BIC, AIC, and df to the table
        bic_table2 <- rbind(bic_table2, data.frame(model = option, BIC = bic_value, AIC = aic_value))
    } else {
        # If there's an error, store NA for BIC, AIC, and df
        bic_table2 <- rbind(bic_table2, data.frame(model = option, BIC = NA, AIC = NA))
    }
}
```


## Let's do post-hoc tests on genet+day_numeric
```{r}
model_growth <- lme(avg_size_log ~  genet + day_numeric , random = ~1 | genet_tank, data = all_growth, correlation = corAR1(form = ~ 1 | genet_tank))
summary(model_growth)

model_growth2 <- lme(avg_size_log ~  treatment+genet + day_numeric , random = ~1 | genet_tank, data = all_growth, correlation = corAR1(form = ~ 1 | genet_tank))
summary(model_growth)

#anova(model_growth, model_growth2)

emm_trends <- emtrends(model_growth, specs = "genet", var = "day_numeric")
emm_trends
emm_genet <- emmeans(model_growth, specs = "genet")
emm_genet
pairs(emm_genet, adjust = "tukey") # Tukey is a common method for pairwise comparisons
```
...
anova(model_beta5, model_beta1)
BIC_check <- BIC(model_beta1, model_beta2, model_beta3, model_beta4, model_beta5)
```
...
BIC_compare <- BIC(model_2, model_3, model_4)
...
## "Calculate" the BIC
```{r}
model <- permanova_mhw_treatment_genet
rss <- model$aov.tab["Residuals", "SumsOfSqs"]
n <- nrow(all_permanova)
k <- nrow(model$aov.tab) - 1
bic <- n * log(rss / n) + k * log(n)
```

```{r}
bic_permanova_mhw_treatment_genet_day <- bic
```
...
# PERMANOVA on 6 time points: start/mid/end of MHW, start/mid/end of recovery
```{r}
all_simple <- all %>%
  filter(date >= ymd("2023-10-06"), #remove 10-06 to make # days between mhw/recovery equivalent
         !is.na(avg_size)) %>%
...
permanova_mhw_genet2 <- adonis(distance_matrix ~ mhw_week+genet,
                                  data = all_simple,
                                  permutations = 999)
permanova_mhw_genet2$aov.tab #R2 = 0.74
...
permanova_mhw_treatment2 <- adonis(distance_matrix ~ mhw_week+treatment,
                                  data = all_simple,
                                  permutations = 999)
...
rss <- model$aov.tab["Residuals", "SumsOfSqs"]
n <- nrow(all_simple)
k <- model$aov.tab["Total", "Df"]-model$aov.tab["Residuals", "Df"]
bic <- n * log(rss / n) + k * log(n)

bic_permanova_mhw_treatment_genet_day <- bic
```

## Function to calculate adjusted R-squared to compare models
```{r}
...
```{r}
calculate_adjusted_r_squared(permanova_mhw_treatment_genet5)
permanova_mhw_treatment_genet$aov.tab

calculate_adjusted_r_squared(permanova_mhw_treatment_genet2)
permanova_mhw_treatment_genet2$aov.tab
...
## Let's try not cheating, and calculate the BIC as Chris requested

## adonis()
```{r}
# Step 1: Put all your models in a named list
models <- list(
  mhw = permanova_mhw,
  treatment = permanova_treatment,
  genet = permanova_genet,
  mhw_genet = permanova_mhw_genet,
  mhw_genet2 = permanova_mhw_genet2,
  mhw_treatment = permanova_mhw_treatment,
  mhw_treatment2 = permanova_mhw_treatment2,
  mhw_treatment_genet = permanova_mhw_treatment_genet,
  mhw_treatment_genet2 = permanova_mhw_treatment_genet2,
  mhw_treatment_genet3 = permanova_mhw_treatment_genet3,
  mhw_treatment_genet4 = permanova_mhw_treatment_genet4,
  mhw_treatment_genet5 = permanova_mhw_treatment_genet5
)

# Step 2: Set number of observations
n <- nrow(all_simple)

# Step 3: Loop through models and compute BIC
...
for (model_name in names(models)) {
  ...
  k <- model$aov.tab["Total", "Df"]-model$aov.tab["Residuals", "Df"]
  bic <- n * log(rss / n) + k * log(n)
  r_squared <- sum(model$aov.tab[1:k, "R2"])  # same as 1 - Residual R2
...
# Step 4: View or sort by BIC
print(bic_table)
```

## adonis2()
```{r}
permanova_mhw <- adonis2(distance_matrix ~ mhw_week,
                        data = all_simple,
                        permutations = 999, by=NULL)

permanova_treatment <- adonis2(distance_matrix ~ treatment,
                               data = all_simple,
                               permutations = 999, by=NULL)

permanova_genet <- adonis2(distance_matrix ~ genet,
                           data = all_simple,
                           permutations = 999, by=NULL)

permanova_mhw_genet <- adonis2(distance_matrix ~ mhw_week*genet,
                               data = all_simple,
                               permutations = 999, by=NULL)

permanova_mhw_treatment <- adonis2(distance_matrix ~ mhw_week*treatment,
                                  data = all_simple,
                                  permutations = 999, by=NULL)

permanova_mhw_treatment_genet <- adonis2(distance_matrix ~ mhw_week*treatment*genet,
                                  data = all_simple,
                                  permutations = 999, by=NULL)

permanova_mhw_treatment_genet2 <- adonis2(distance_matrix ~ mhw_week+treatment+genet,
                                  data = all_simple,
                                  permutations = 999, by=NULL)

permanova_mhw_treatment_genet3 <- adonis2(distance_matrix ~ mhw_week*treatment+genet,
                                  data = all_simple,
                                  permutations = 999, by=NULL)

permanova_mhw_treatment_genet4 <- adonis2(distance_matrix ~ mhw_week+treatment*genet,
                                  data = all_simple,
                                  permutations = 999, by=NULL)

permanova_mhw_treatment_genet5 <- adonis2(distance_matrix ~ mhw_week*genet+treatment,
                                  data = all_simple,
                                  permutations = 999, by=NULL)

# Step 1: Put all your adonis2() models in a named list
models <- list(
  mhw = permanova_mhw,
  treatment = permanova_treatment,
  genet = permanova_genet,
...
for (model_name in names(models)) {
  ...
  # Only for adonis2() models (since we're assuming they have 'SumOfSqs')
  rss <- model$SumOfSqs[2]
  n <- sum(model$Df[3])  # or extract from model$Df["Total"]
  k <- model$Df[1]
  r_squared <- sum(model$SumOfSqs[1:k]) / sum(model$SumOfSqs)
  
  # Calculate BIC
  bic <- n * log(rss / n) + k * log(n)
  
  # Add the results to the bic_table
  bic_table2 <- rbind(bic_table2, data.frame(
    model = model_name,
    rss = rss,
    k = k,
    bic = bic,
...
bic_model2 <- n * log(rss / n) + k * log(n)

```

## Let's visualize our distance matrix
...
pcoa_df <- data.frame(
  ...
  mutate(mhw_week = factor(mhw_week, levels = c("during.0", "during.5", "during.9", "after.0", "after.4", "after.8"),
                           labels = c("During (Week 0)", "During (Week 5)", "During (Week 9)", "After (Week 0)", "After (Week 4)", "After (Week 8)")))

centroids <- pcoa_df %>%
  group_by(treatment, mhw_week) %>%
  summarise(
...
library(viridis)
ggplot(pcoa_df, aes(x = PC1, y = PC2, color = mhw_week, shape = mhw_week)) +
  stat_ellipse(aes(fill = mhw_week,color=mhw_week), geom = "polygon", alpha = 0.2) +
  geom_point(size = 3, alpha=0.6) +
  scale_fill_grey(start = 0.6, end = 0.1) +
  scale_color_grey(start = 0.6, end = 0.1) +
  scale_x_continuous(breaks = c(-0.6, -0.3, 0, 0.3)) +
  #                   breaks =) +
  #scale_fill_manual(values = rev(c("#0072B2","#2F77A2", "#5D7C92", "#8C817F", "#BA866F", "#D55E00"))) +
  #scale_color_manual(values = rev(c("#0072B2","#2F77A2", "#5D7C92", "#8C817F", "#BA866F", "#D55E00"))) +
#"#003366","#00558F","#0072B2","#4D9DCB", "#9DC6E2","#D0E6F5")) +
  #geom_point(data = centroids, aes(x = centroid_PC1, y = centroid_PC2), shape = 4, size = 4, stroke = 1.5) +  # shape 4 = X
  #geom_path(data = centroids, aes(x = centroid_PC1, y = centroid_PC2, group = treatment),color = "black", linewidth = 0.8, arrow = arrow(length = unit(0.2, "cm"))) +
    labs(title = "",
       x = "PCoA 1",
       y = "PCoA 2",
       color= "Time period",
       shape="Time period") +
  guides(fill=FALSE) +
  facet_wrap(~treatment, labeller = labeller(treatment = c(cold = "Ambient", severe = "Severe", extreme = "Extreme"))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        text = element_text(size = 20),
        strip.background = element_rect(fill = "white", color = "black"))

#ggsave(here("experiment", "figures", "dissertation","permanova.png"), width=12, height=7)
```

### NMDS 
```{r}
nmds_result <- metaMDS(distance_matrix, distance = "gower", k = 2, trymax = 100)

# Extract NMDS scores
nmds_df <- as.data.frame(scores(nmds_result))
nmds_df <- cbind(nmds_df, all_simple)
nmds_df$group <- interaction(nmds_df$treatment, nmds_df$genet)

# Plot NMDS
ggplot(nmds_df, aes(x = NMDS1, y = NMDS2, color = mhw_week)) +
  geom_point(size = 3) +
  stat_ellipse() +
  labs(title = "NMDS of Gower Distance") +
  theme_minimal()
```
...
#anosim_result <- anosim(distance_matrix, grouping = all_simple$treatment, permutations = 999)
#anosim_result <- anosim(distance_matrix, grouping = all_simple$genet, permutations = 999)
#anosim_result <- anosim(distance_matrix, grouping = all_simple$mhw_week, permutations = 999)
