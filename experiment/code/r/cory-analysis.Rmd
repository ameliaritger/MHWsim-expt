...
alive <- read_csv(here("experiment", "data", "mortality.csv")) %>% #each file will be read in, specify which columns you need read in to avoid any error
  clean_names() %>%
  mutate(date = if_else(mdy(date) == "2024-01-31", mdy(date)-days(1), mdy(date))) #correct for date discrepancy between measurements
  #arrange(date) %>%
...
all <- full_join(open, alive, by=join_by(date==date, tank==tank, genet==genet)) %>%
  ...
  mutate(removed_cum = if_else(is.na(cumsum(removed)), 0, cumsum(removed)),# generate cumulative number of polyps removed, to represent total number of polyps excluding removal
         ...
         growth_rate_cont = cumsum(diff_n_cont)) %>% #overwrite n value to incorporate cumulative number of removed polyps
...
```{r}
label_plot <- c("0.136", "0.0972", "0.0734", "0.0614", "0.085",
                "0.154", "0.101", "0.0299", "0.124", "0.0577",
                "0.175", "0.151", "0.0603", "0.141", "0.0978",
                "0.00128", "0.0119", "0.026", "0.00843", "-0.0104",
                "0.0102", "0.00652", "0.00881", "0.00421", "-0.0117",
                "-0.0104", "-0.000895", "0.0217", "0.0139", "-0.0198")
