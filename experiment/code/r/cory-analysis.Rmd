...
eq <- eq %>%
#add back genet column
  mutate(genet = model$genet)

ggplot(df, aes(x, y)) +
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ poly(x, 2), se = FALSE) +
  geom_label(x = 5, y = 35, label = eq)
...
ggplot(all[all$mhw == "after" & all$treatment == "extreme", ], aes(x = date, y = growth_rate, group = genet)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", formula = y ~ x, se = F) +
  ...
  #stat_poly_eq(use_label("eq"), na.rm = TRUE, coef.keep.zeros = TRUE) +
