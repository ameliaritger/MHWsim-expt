...
```
Residual plot looks off, let's try NB GLMM...
...
all_count_plot <- all_count %>%
  select(date, tank, genet, genet_tank, treatment, n_true) %>%
  ...
  summarize(mean = mean(n_true),
            ...
            lower = mean - ci,
            upper = mean + ci) %>%
  select(date, treatment, genet, mean, sd, lower, upper)
...
ggplot(all_count_plot, aes(x=date,color=genet)) +
  geom_point(aes(y=logmean)) +
  facet_wrap(~treatment) +
  labs(x = "Date",
       y = "Count")

# Get predictions
all_count$predicted <- predict(count_glmm14)

# Plot
ggplot(all_count, aes(x = n_true, y = predicted)) +
  geom_point() +
...
We need a nonlinear regression, the curves are logistic for sure
```{r}
