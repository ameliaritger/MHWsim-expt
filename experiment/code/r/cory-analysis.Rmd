...
### Check distribution
```{r}
hist(all_count$growth_rate_cont, breaks = seq(min(all_count$growth_rate_cont), max(all_count$growth_rate_cont) + 0.5, by = 1), 
     main = "Histogram of Growth Rate", xlab = "Growth Rate", 
     ylab = "Frequency", col = "lightblue")

hist(all_count$n_true, breaks = seq(min(all_count$n_true), max(all_count$n_true) + 0.5, by = 1), 
...
```{r}
# Fit a Poisson model
poisson_model <- glmer(n_true ~ treatment + day_numeric + (1|tank), family = poisson, data = all_count)

# Fit a Negative Binomial model
nb_model <- glmer.nb(n_true ~ treatment + day_numeric + (1|tank), data = all_count)
...
expected_counts <- dpois(as.numeric(names(observed_counts)), lambda) * sum(observed_counts)
# Perform the Chi-squared test
chisq.test(observed_counts, p = expected_counts / sum(expected_counts))

# Calculate mean and variance of growth_rate
mean_growth <- mean(all_count$growth_rate_cont)
var_growth <- var(all_count$growth_rate_cont)
# Check ratio
overdispersion_ratio <- var_growth / mean_growth
...
# Calculate mean and variance of growth_rate
mean_growth <- mean(all_count$n_true)
var_growth <- var(all_count$n_true)
# Check ratio
overdispersion_ratio <- var_growth / mean_growth
print(overdispersion_ratio) #growth rate is overdispersed
```

### Check histogram of count data
```{r}
