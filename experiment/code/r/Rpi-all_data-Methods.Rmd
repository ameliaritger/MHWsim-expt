...
temp_compare <- temp_compare %>%
  mutate(diff = round(avg_temp - avg_temp_sub, 3),
         above_below = ifelse(diff > 0, "above", 
                              ifelse(diff< 0, "below", "equal")))
...
#plot the data
ggplot(temp_compare, aes(x=timestamp)) +
  geom_line(aes(y=0), color="black", size=1, linetype="dashed") +
  geom_point(aes(y=diff, color=above_below, shape=above_below), size=1.5) +
  ...
  scale_color_manual(values = c("above" = "#0072B2", "below" = "#D55E00", equal="#E69F00"),
                     labels = c("above" = "DS18B20 > HOBO", "below" = "DS18B20 < HOBO", "equal" = "DS18B20 = HOBO")) +
  scale_shape_manual(values = c("above" = 17, "below" = 15, equal=16),
                     labels = c("above" = "DS18B20 > HOBO", "below" = "DS18B20 < HOBO", "equal" = "DS18B20 = HOBO")) +
  labs(y = expression(paste("Temperature difference (", degree, "C)")),
       x = "Date and time") +
  theme_bw() +
...
#ggsave(here("experiment", "figures", "dissertation", "hoboTempDiff.png"), width=8, height=5)
